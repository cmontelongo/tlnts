...
<jsp:include page="menuSide2.jsp">
	<jsp:param name="titulo" value="Alta Salon" />
</jsp:include>
<%
...
<iframe id="listado" src="./salon_listadoUltimos.jsp" style="min-width:800px; width:90%;" frameborder="0"></iframe>
</center>
<script>
$('#guarda').button();
$('#guarda-icon').button({
...
var grupoDisponible = <%= salonAction.obtenerSalonJson() %>;

$( "#salon" ).autocomplete({
	source: grupoDisponible,
    select: function (event, ui) {
        $("#salon").val(ui.item.label); // display the selected text
        $("#salonId").val(ui.item.id); // save selected id to hidden input
    }
});
